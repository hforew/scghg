...
## load the main function which will build the model and load all components
include("main_model_park_mortality.jl")

# ## load helper mcs and scc functions
include("main_mcs_park_mortality.jl")
include("scc_park_mortality.jl")
...
## use the get_modified_model function here
m = get_modified_model(version=version); # set verion function argument to version variable defined above

println("start estimation")
## estimate scghg  --> function compute_scc is from MimiGIVE package. used for replication
#  results = 
...
mkpath(joinpath(@__DIR__, "../output/scghgs/full_distributions/$gas"))
