...
include("utils/scc_streaming_park_mortality.jl")
    ...
    if discount_rates!==nothing
        ...
        for dr in discount_rates
            ...
            elseif dr.ew==:gdp_country
                ...
                park_mortality_marginal_damages = mm[:fire_mortality_damages, :mortality_costs] .* scc_gas_molecular_conversions[gas] # country
                ...
                pc_gdp_for_park_mortality = mm.base[:PerCapitaGDP, :pc_gdp]
                n_regions_for_park_mortality = size(pc_gdp_for_park_mortality, 2)
                park_mortality_scc_in_utils = sum(
                    park_mortality_marginal_damages[i,r] / pc_gdp_for_park_mortality[i,r]^dr.eta * 1/(1+dr.prtp)^(t-year)
                    for (i,t) in enumerate(_model_years), r in 1:n_regions_for_park_mortality if year<=t<=last_year
                )
                ...
                total_utils = 
                    ...
                    (mm.base[:DamageAggregator, :include_park_mortality]   ? park_mortality_scc_in_utils : 0.) +
...
function modified_post_trial_func(mcs::SimulationInstance, trialnum::Int, ntimesteps::Int, tup)
    ...
    if options.compute_sectoral_values
        ...
        park_mortality_mds = post_trial_mm[:DamageAggregator, :park_mortality_damage]
        ...
        if options.compute_domestic_values
            ...
            park_mortality_mds_domestic = post_trial_mm[:DamageAggregator, :park_mortality_damage_domestic]
        end
    end
    ...
    if options.save_md
        ...
        if options.compute_sectoral_values
            ...
            md_values[(region=:globe, sector=:park_mortality)][trialnum, :]     = park_mortality_mds[_damages_idxs]
        ...
        if options.compute_domestic_values
            ...
            if options.compute_sectoral_values
                ...
                md_values[(region=:domestic, sector=:park_mortality)][trialnum, :]     = park_mortality_mds_domestic[_damages_idxs]
    ...
    for dr in discount_rates
        ...
        if isnothing(dr.ew)
            ...
            if options.compute_sectoral_values
                ...
                scc = sum(df .* park_mortality_mds[year_index:last_year_index])
                scc_values[(region=:globe, sector=:park_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
                ...
                if options.certainty_equivalent
                    ...
                    intermediate_ce_scc = sum(df_ce .* park_mortality_mds[year_index:last_year_index])
                    intermediate_ce_scc_values[(region=:globe, sector=:park_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = intermediate_ce_scc
                ...
                if options.compute_domestic_values
                    ...
                    scc = sum(df .* park_mortality_mds_domestic[year_index:last_year_index])
                    scc_values[(region=:domestic, sector= :park_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
                    ...
                    if options.certainty_equivalent
                        ...
                        intermediate_ce_scc = sum(df_ce .* park_mortality_mds_domestic[year_index:last_year_index])
                        intermediate_ce_scc_values[(region=:domestic, sector= :park_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = intermediate_ce_scc
        ...
        elseif dr.ew==:gdp_region || dr.ew==:gdp_country
            if dr.ew==:gdp_region
                ...
                park_mortality_marginal_damages = post_trial_mm[:DamageAggregator, :damage_park_mortality_regions] # fund regions 
            ...
            elseif dr.ew==:gdp_country
                ...
                park_mortality_marginal_damages = post_trial_mm[:fire_mortality_damages, :mortality_costs]
            ...
            park_mortality_scc_in_utils = sum(
                park_mortality_marginal_damages[i,r] / pc_gdp[i,r]^dr.eta * 1/(1+dr.prtp)^(t-year)
                for (i,t) in enumerate(_model_years), r in 1:n_regions if year<=t<=last_year
            )
            ...
            total_utils =
                ...
                (base[:DamageAggregator, :include_park_mortality]   ? park_mortality_scc_in_utils : 0.) +
            ...
            if options.compute_sectoral_values
                ...
                scc = base[pc_gdp_component_name, :pc_gdp][year_index,normalization_region_index]^dr.eta * park_mortality_scc_in_utils
                scc_values[(region=:globe, sector=:park_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
                ...
                if options.certainty_equivalent
                    ...
                    intermediate_ce_scc_values[(region=:globe, sector=:park_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = park_mortality_scc_in_utils
        ...
        elseif dr.ew==:consumption_region || dr.ew==:consumption_country
            ...
            park_mortality_scc_in_utils = sum(
                park_mortality_marginal_damages[i,r] / pc_consumption[i,r]^dr.eta * 1/(1+dr.prtp)^(t-year)
                for (i,t) in enumerate(_model_years), r in 1:n_regions if year<=t<=last_year
            )
            ...
            if options.compute_sectoral_values
                ...
                scc = base[net_cpc_component_name, :net_cpc][year_index,normalization_region_index]^dr.eta * park_mortality_scc_in_utils
                scc_values[(region=:globe, sector=:park_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
...
function _compute_modified_scc_mcs(mm::MarginalModel, 
    ...
    sectors = compute_sectoral_values ? [:total,  :cromar_mortality, :agriculture, :energy, :park_mortality, :slr] : [:total]
    ...
    if compute_disaggregated_values
        ...
        mkpath(joinpath(top_path, "damages_park_mortality"))		
