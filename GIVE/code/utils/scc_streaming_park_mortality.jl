...
function _modified_stream_disagg_damages(m::Mimi.Model, output_dir::String, trialnum::Int, streams::Dict)	
    ...
    park_mortality_damages = getdataframe(m, :DamageAggregator, :damage_park_mortality) |> 	
                        @filter(_.time > 2019) |> 	
                        @rename(:damage_park_mortality => :damages) |>	
                        DataFrame |>	
                        i -> insertcols!(i, 1, :trialnum => trialnum)	
    ...
    for country in unique(park_mortality_damages.country)	
        filename = joinpath("$output_dir/results/disaggregated_values/damages_park_mortality/$(country).csv")	
        trial_df = park_mortality_damages |> @filter(_.country == country) |> @select(:trialnum, :time, :damages) |> DataFrame	
        if haskey(streams, filename)	
            write(streams[filename], trial_df)	
        else	
            streams[filename] = savestreaming(filename, trial_df)	
...
function _modified_stream_disagg_md(m_base::Mimi.Model, m_modified::Mimi.Model, ciam_base::Union{Nothing, Mimi.ModelInstance}, md_ciam::Union{Nothing, Array}, 	
    ...
    md_park_mortality   = (view(m_modified[:DamageAggregator, :damage_park_mortality], _damages_idxs,:) .- view(m_base[:DamageAggregator, :damage_park_mortality], _damages_idxs,:)) .* 1e9 .* gas_units_multiplier	
    ...
    md_country_df = DataFrame(md_cromar_mortality .+ md_energy .+ md_park_mortality .+ md_ciam_all_countries, dim_keys(m_base, :country)) |>	
                    i -> insertcols!(i, 1, :time => _damages_years) |>	
                    i -> stack(i, Not(:time)) |>	
