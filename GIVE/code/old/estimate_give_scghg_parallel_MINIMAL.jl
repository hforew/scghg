# Note: when running this file, you will want to have launch Julia with
# `julia --threads=<n>` where <n> is the number of threads you want to use.
# or `julia --threads=auto` to use all available threads.
# running this with 6 concurrent threads for me took 9 GB of memory
# to avoid running out of memory, you may need to limit the number of year-gas combinations you run at a time
# and reset Julia (open and close) between each run
...
## automatically download data dependencies (rffsps)
println("Loading data dependencies")
...
gas_year_combos = Iterators.product(gases, years) |> collect
println("Running $(length(gas_year_combos)) combinations in $(Threads.nthreads()) threads")

Threads.@threads for (gas, year) in gas_year_combos
    ...
    scghgs_mean |> save(joinpath(@__DIR__, "../output/scghgs/sc-$gas-$damages-$year-$n.csv"));
