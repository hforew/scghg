...
@defcomp DamageAggregator_FireDamages begin
    ...
    include_park_mortality = Parameter{Bool}(default=true) # new damage sector
    ...
    damage_park_mortality = Parameter(index=[time,country], unit="US\$2005/yr") # new sector
    ...
    damage_park_mortality_regions = Parameter(index=[time,fund_regions], unit="US\$2005/yr") # new sector. takes input for each value in index (time, region)
    ...
    park_mortality_damage         = Variable(index=[time], unit="US\$2005/yr") # new sector
    ...
    park_mortality_damage_domestic          = Variable(index=[time], unit="US\$2005/yr") # new sector
    ...
    function run_timestep(p, v, d, t)
        ...
        for r in d.fund_regions 
            v.total_damage_regions[t,r] = # for each region and year, set total damage equal to sum of these damages
                (p.include_cromar_mortality ? p.damage_cromar_mortality_regions[t,r] : 0.) +
                (p.include_park_mortality   ? p.damage_park_mortality_regions[t,r] : 0.) + # new sector if park_mortality  true, then add it
        ...
        v.total_damage_countries[t,:] =
            ...
            (p.include_park_mortality   ? p.damage_park_mortality[t,:] : fill(0., num_countries)) + # new sector
        ...
        v.park_mortality_damage[t]        = sum(p.damage_park_mortality[t,:]) # new sector. sum damage for year across countries
        ...
        v.total_damage[t] =
            ...
            (p.include_park_mortality   ? v.park_mortality_damage[t] : 0.) + # new sector
        ...
        v.park_mortality_damage_domestic[t]           = sum(p.damage_park_mortality[t, v.domestic_idxs_country_dim_int])
        ...
        v.total_damage_domestic[t] =
            ...
            (p.include_park_mortality ? v.park_mortality_damage_domestic[t] : 0.) +
